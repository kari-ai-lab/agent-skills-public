...
5. **Naming** — are identifiers descriptive without being unwieldy. Route the actual naming judgment to `coding-standards-and-design-patterns.md`'s Design for the Human Reader First section — grounded directly in Google's own AIP-192 documentation standard ("the API surface... will be the only things a user has"), the same reasoning applies to a function or class name: it's frequently the only thing a future reader has before deciding whether to trust it.
6. **Comments** — do they explain *why*, not restate what the code already says (line up directly with this workspace's own default no-comments-unless-non-obvious-why rule, and with `coding-standards-and-design-patterns.md`'s same AIP-192-grounded reasoning).
