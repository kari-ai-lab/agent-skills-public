...
- **Candidate name(s)**, or a blank slate needing generation — run the Naming Interview below when no candidates exist yet.
...
- **When starting from a blank slate:** a completed five-question interview transcript, and a generated shortlist of 5-8 candidates (mixing word combinations and merged/blended coinages), each already run through the Spectrum + 4Cs checks below — not a raw, unscored brainstorm list.
...
- **Standalone (always works):** The user supplies candidate names, or a blank slate the skill generates from via the Naming Interview, plus the target segment/tone directly; the skill classifies and scores from that alone.
...
## Naming Interview — Generating Candidates From a Blank Slate

When no candidate names exist yet, run this five-question interview before jumping into the Decision Framework below. Ask one question at a time — don't front-load all five, since each answer sharpens the context for the next question. This interview format itself is workspace-authored (not drawn from an external source); the naming theory it feeds into is the same Spectrum/4Cs sourcing already cited in this skill.

**The five questions, in order:**

1. **Vision** — "What is this product ultimately trying to become? What does 'great' look like once it's real?" If a target-state description already exists via `target-state-vision-and-phased-roadmap.md`, reuse it — don't re-derive it here.
2. **Target Client** — "Who is the primary buyer or user this name has to resonate with?" Reuse `../product/user-persona-development.md` if it already exists.
3. **Market** — "What category or market is this competing in, and against whom?" Reuse `../product/competitor-analysis-synthesizer.md` or `product-and-solution-portfolio-definition.md` if available — this doubles as input to the collision check in Inputs Required.
4. **Emotional Association** — "What should someone feel or think in the moment they first hear this name?" This is raw material for the Communication axis.
5. **Five-Word Free Association** — "Give me five words you associate with this product. Don't overthink it — first instinct, not a brainstorm." This is raw material for both the Construction and Character axes: unfiltered association surfaces real signal that a deliberate brainstorm tends to edit away.

**Generating candidates from the five answers:**

Produce two distinct types of candidate, both drawn directly from the interview answers — never invented independently of them:

- **Word combinations** — pair two of the five associated words, or one associated word with a market/vision term, into a compound or short phrase. These land toward the Suggestive or Descriptive end of the Spectrum — still legible, still means something on inspection.
- **Merged/blended coinages** — fuse fragments of two of the five words (or an associated word with a vision/market term) into a new word with no literal dictionary meaning. This is the same **Fanciful/Coined** Spectrum category and **lexical blend/portmanteau** Construction type already defined above — the interview's raw material is what makes a coined word feel *earned* rather than arbitrary, even though it has no literal meaning once assembled.

Produce 5-8 candidates total, mixing both types — never all combinations or all blends. For each, immediately run the Spectrum classification and 4Cs scoring from the Decision Framework below, so the user sees the real trade-off rather than a raw, unscored brainstorm list.

**The confirmation loop:**

Present the scored candidate list and ask the user to do exactly one of three things:

1. **Confirm** one candidate as final — proceed to the Decision Framework's trade-off statement and recommendation.
2. **Adjust** a candidate (respell it, shorten it, swap a fragment) — re-score the adjusted version before treating it as final; an adjusted name is a new candidate, not an automatic pass.
3. **Reject and regenerate** — ask specifically what didn't work (too generic, too strange, wrong tone, too close to a competitor) and produce a new round constrained by that specific feedback, rather than a second undirected batch.

Loop until the user explicitly confirms a final candidate (or a short shortlist) — never treat the first round of recommendations as final without that confirmation.
...
2. **Generate or collect candidates.** Run the Naming Interview above when starting from a blank slate.
...
```markdown
## Naming Interview (skip this block if candidates were already supplied)

1. **Vision:** [answer]
2. **Target Client:** [answer]
3. **Market:** [answer]
4. **Emotional Association:** [answer]
5. **Five-Word Free Association:** [word 1, word 2, word 3, word 4, word 5]

### Generated Candidates
| Candidate | Type | Built from |
| --- | --- | --- |
| [Name] | Word combination | [which two answer-words/terms] |
| [Name] | Merged/blended coinage | [which word fragments] |
...
and tone, one or more candidate names OR a blank slate needing generation,
and any known naming collisions to avoid.

IF NO CANDIDATE NAMES EXIST YET, run the interview first, one question at a
time:
  1. Vision — what is this product ultimately trying to become?
  2. Target Client — who is the primary buyer/user?
  3. Market — what category, and against whom?
  4. Emotional Association — what should someone feel/think on first hearing
     the name?
  5. Five-Word Free Association — five words associated with the product,
     first instinct, not a considered brainstorm.

  From those five answers, generate 5-8 candidates, mixing two types:
    - Word combinations: pair two answer-words (or an answer-word with a
      market/vision term) into a compound or short phrase.
    - Merged/blended coinages: fuse fragments of two answer-words (or an
      answer-word with a market/vision term) into a new word with no
      literal meaning — this is the Fanciful/Coined Spectrum category and
      the lexical-blend Construction type.
  Never invent a candidate that isn't traceable back to a specific
  interview answer.

For each candidate name (whether supplied or interview-generated):
...
If interview-generated, present the scored shortlist and stop — ask the user
to Confirm one, Adjust one (re-score the adjusted version before it counts
as final), or Reject and regenerate with specific feedback on what didn't
work. Do not proceed to the trade-off statement and recommendation below
until the user has explicitly confirmed a final candidate.
...
- Never treat an interview-generated shortlist as final without the user's
  explicit confirmation, adjustment, or regeneration request.

Now evaluate:
Brand architecture classification: $BRAND_ARCHITECTURE_CLASSIFICATION
Target segment/tone: $TARGET_SEGMENT_TONE
Candidate names (or "generate from interview"): $CANDIDATE_NAMES
Interview answers (if generating): $INTERVIEW_ANSWERS
...
- [ ] When generating from a blank slate, all five interview questions were asked (one at a time, not front-loaded) before any candidate was generated.
- [ ] Every generated candidate traces back to a specific interview answer — no candidate invented independently of the five-word association or the other answers.
- [ ] The generated shortlist mixes word combinations and merged/blended coinages — not all of one type.
- [ ] An interview-generated shortlist was not treated as final without the user explicitly confirming, adjusting, or requesting regeneration.
...
**Not externally sourced, flagged rather than overclaimed:** the five-question Naming Interview format (Vision / Target Client / Market / Emotional Association / Five-Word Free Association) and the word-combination/merged-coinage generation technique are workspace-authored, built at direct user request rather than adapted from a named external methodology. The naming theory the generated candidates are evaluated against (Spectrum of Distinctiveness, the 4Cs) is the same externally sourced material cited above.
...
- `../product/value-proposition-canvas.md`, `../product/user-persona-development.md` — supply the target segment/tone input to the Character axis, and the Naming Interview's Target Client answer when already available.
- `target-state-vision-and-phased-roadmap.md` — supplies the Naming Interview's Vision answer when a target-state description already exists; reuse it rather than re-deriving.
- `../product/competitor-analysis-synthesizer.md` — supplies the Naming Interview's Market answer and doubles as input to the collision check, when available.
