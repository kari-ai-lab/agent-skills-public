...
- **No YAML frontmatter / auto-triggering in this workspace's skills.** Anthropic's description-optimization phase (20 trigger-eval queries, 60/40 train/test split, an automated optimization loop tuning the frontmatter `description` for Claude's Skill-loading system) does not directly transfer, since this workspace's skills are manually referenced via `INDEX.md` and folder `README.md`s rather than auto-loaded off a description field. What DOES transfer: write each skill's Objective section clearly enough that a reader scanning the folder's `README.md` bullet can tell at a glance whether this is the right skill for their situation — that bullet is functionally this workspace's version of a triggering description, even without the automated tooling.
