...
description: "Use to write a product requirements document once discovery is done: a fillable section-by-section template from executive summary through executive ask, with an AI-feature supplement, business case, goals and metrics, and Gherkin acceptance criteria."
---
...
**Minimum viable input:** the discovery output. If discovery was never run, still draft — but mark every section it would have evidenced as **Assumption**, so the PRD advertises its own weak points instead of presenting guesses in the same voice as findings.
...
- **Guardrail metric(s):** For each goal above, at least one metric tracked
  specifically to catch a side effect the fix could cause elsewhere (e.g. a
  conversion fix that quietly raises support load, a loss-reduction fix that
  quietly raises false declines for one segment). Distinct from "explicitly
  not measured" below — a guardrail is actively watched precisely because
  the goal could move it, not excluded from scope. See
  `pm-pipeline-checkpoints.md` Checkpoint 2 if a goal has none.
...
- If discovery has not been run → `product-requirements-discovery-questionnaire.md` first; this template records thinking, it does not test it.
- When a feature needs implementation-ready detail → `functional-requirements-document-template.md`.
- To take features into a tracker → `../delivery/jira-epic-builder.md`.
