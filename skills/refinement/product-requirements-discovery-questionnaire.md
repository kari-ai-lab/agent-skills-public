---
name: product-requirements-discovery-questionnaire
description: "Use before writing a PRD: an evidence-demanding interview covering business overview, business case, constraints, scope, problem, goals and metrics, features, edge cases, open questions, launch plan and the executive ask, so gaps surface before drafting."
---
...
**Minimum viable input:** the initiative name and the problem it claims to solve. Unanswered sections are the deliverable — this skill exists to surface gaps, so a mostly-empty result is a successful run, not a failed one. Record who owns each unanswered question rather than leaving it unattributed.
...
- Once the answers hold → `product-requirements-document-template.md` to draft.
- If a vendor or build-versus-buy decision is live → `pm-pipeline-checkpoints.md` alongside, before Constraints and Scope are locked.
- If the initiative never cleared intake → `../strategy/product-proposal-viability-scoring.md` first.
