---
name: functional-requirements-document-template
description: "Provides the canonical Functional Requirements Document template: how the system should build it and make it work — step-by-step workflows, system logic, data rules, and error handling."
---
...
**Minimum viable input:** the PRD feature this elaborates. Without an upstream PRD, write the FRD anyway but record which product decisions are being made here by default — an FRD silently deciding product questions is the failure this handoff exists to prevent.
...
- If no PRD exists yet → `product-requirements-document-template.md` first.
- To take the result into a tracker → `../delivery/jira-epic-builder.md`.
- If the feature touches card data → `../governance/pci-requirements-for-product-owners.md`; personal data → `../governance/privacy-law-awareness-for-product-development.md`.
